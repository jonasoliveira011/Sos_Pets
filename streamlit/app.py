...
def query_bedrock(message, session_id="", model_params=None, context="", conversation_history=None):
    ...
    try:
        prompt = generate_chat_prompt(message=message, conversation_history=conversation_history, context=context)
...
def check_password():
    ...
    def password_entered():
        ...
        hmac.compare_digest(st.session_state["password"].strip(), "grupo1"):
...
if check_password():
    ...
    with st.sidebar:
        ...
        st.button("🔄 Nova Pesquisa", on_click=create_new_chat, use_container_width=True)
        
        st.divider()
        
        st.markdown("### Minhas Pesquisas")
